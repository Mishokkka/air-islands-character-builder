...
assert.match(source, /REBOOT занимает Quarter Day, восстанавливает WITS как сон, 1 EMPATHY, полностью снимает OVERLOAD/u, "Описание REBOOT в UI должно совпадать с правилами Механического Тела");
assert.match(source, /MAINTENANCE занимает Quarter Day и снимает 1D6 OVERLOAD/u, "Описание MAINTENANCE в UI должно совпадать с правилами Механического Тела");
assert.doesNotMatch(source, /Для восстановления используются REBOOT, MAINTENANCE/u, "UI не должен намекать, что REBOOT или MAINTENANCE восстанавливают физический Damage");
